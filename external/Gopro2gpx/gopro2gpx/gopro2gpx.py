...
def BuildGPSPoints(data, skip=False, skipDop=False, dopLimit=2000, timeShift=0):
    ...
    for d in data:
        ...
        if d.fourCC == 'SCAL':
            ...
            if d.data.lon == 0 and d.data.lat == 0:
                print("Warning: Skipping empty point")
                stats['empty'] += 1
                continue
            if GPSFIX == 0:
...
def main_core(args):
    ...
    # Ajuste de rutas para el entorno frozen - REMOVED as it is handled in config.py
    ...
    points, gps_start_time, device_name = BuildGPSPoints(data, skip=args.skip, skipDop=args.skip_dop, dopLimit=args.dop_limit, timeShift=args.time_shift)
    
    metadata_start_time = None

    # Get metadata time
    if len(files) > 0 and not args.binary:
        creation_time_str = ffmpegtools.getCreationTime(files[0])
        if creation_time_str:
            try:
                if creation_time_str.endswith('Z'):
                    creation_time_str = creation_time_str[:-1]
                if '.' in creation_time_str:
                     base, micro = creation_time_str.split('.')
                     micro = micro[:6]
                     creation_time_str = f"{base}.{micro}"
                     metadata_start_time = datetime.datetime.strptime(creation_time_str, "%Y-%m-%dT%H:%M:%S.%f")
                else:
                     metadata_start_time = datetime.datetime.strptime(creation_time_str, "%Y-%m-%dT%H:%M:%S")
            except Exception as e:
                print(f"Error parsing creation time: {e}")

    # Display times as requested
    print("-" * 50)
    print("TIME INFORMATION:")
    if metadata_start_time:
        print(f"  File Creation Time (Metadata): {metadata_start_time} UTC")
        try:
            local_time = metadata_start_time.replace(tzinfo=datetime.timezone.utc).astimezone()
            print(f"  File Creation Time (Local):    {local_time}")
        except:
            pass
    else:
        print("  File Creation Time (Metadata): Not found")

    if gps_start_time:
        print(f"  GPS Start Time (Original):     {gps_start_time} UTC")
    else:
        print("  GPS Start Time (Original):     Not found")
    print("-" * 50)

    # Logic requested by user:
    # Always synchronize GPS points to Metadata time if available, to ensure "real precise time".
    
    start_time = gps_start_time

    if metadata_start_time:
        if len(points) > 0:
            # If we have points, we sync them to the metadata time
            # We assume metadata_start_time is the correct "real" start time of the video
            # and we shift the GPS track to match it.
            
            # Use the first point's time to calculate offset, or gps_start_time if available
            ref_time = points[0].time if points else gps_start_time
            
            if ref_time:
                offset = metadata_start_time - ref_time
                # Only apply if offset is significant (> 1 second) to avoid micro-jitter
                if abs(offset.total_seconds()) > 1:
                    print(f"Syncing GPS track to Metadata time. Offset: {offset}")
                    for p in points:
                        p.time += offset
                    start_time = metadata_start_time
                else:
                    print("GPS time matches Metadata time.")
                    start_time = metadata_start_time # Use the clean metadata time
            else:
                 start_time = metadata_start_time
        else:
            # No points, just set the start time
            start_time = metadata_start_time
            print(f"No GPS points. Using Metadata time: {start_time}")
    else:
        if start_time is None and len(points) > 0:
             start_time = points[0].time

    if start_time:
        print(f"Final GPS Track Start Time: {start_time}")
