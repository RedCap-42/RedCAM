...
class FFMpegTools:
    ...
    def getCreationTime(self, fname):
        """
        Gets the creation_time from the file metadata using ffprobe.
        Returns a string in ISO 8601 format (e.g. 2021-03-26T09:44:06.000000Z) or None.
        """
        # Try to get creation_time from format tags
        args = ['-v', 'quiet', '-show_entries', 'format_tags=creation_time', '-of', 'default=noprint_wrappers=1:nokey=1', fname]
        output = self.runCmdRaw(self.ffprobe, args).decode('utf-8').strip()
        
        if output:
            return output
            
        # Fallback: try to get it from the first stream tags if format tags failed
        args = ['-v', 'quiet', '-select_streams', 'v:0', '-show_entries', 'stream_tags=creation_time', '-of', 'default=noprint_wrappers=1:nokey=1', fname]
        output = self.runCmdRaw(self.ffprobe, args).decode('utf-8').strip()
