...
class GoPro2GPXGUI(tk.Tk):
    ...
    def create_principal_widgets(self):
        ...
        tk.Label(routes_frame, text="Video File or Directory:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
    ...
    def select_input_dir(self):
        # If a path has already been selected, use it as the initial directory.
        ...
        if os.path.isdir(initial_dir):
            ...
            initial_dir = os.path.dirname(initial_dir) if initial_dir else self.last_video_dir
        # Ask for file first
        file_path = filedialog.askopenfilename(title="Select Video File", initialdir=initial_dir, filetypes=[("Video files", "*.mp4 *.mov *.avi"), ("All files", "*.*")])
        if file_path:
            ...
            # If no file selected, ask for directory
    ...
    def on_process(self):
        ...
        if not self.input_dir.get():
            messagebox.showerror("Error", "Please select the video directory or file.")
            return
        ...
        if os.path.isfile(input_path):
            if input_path.lower().endswith((".mp4", ".mov", ".avi")):
                ...
                messagebox.showerror("Error", "The selected file is not a valid video file (.mp4, .mov, .avi).")
                return
            ...
            messagebox.showerror("Error", "The input path is neither a valid file nor a directory.")
            return
